...
export default function Settings({ onBack }: SettingsProps) {
  ...
  const Section = ({ title, children }: { title: string; children: React.ReactNode }) => (
    <div>
      <h2 className="text-xs font-bold text-text-secondary uppercase tracking-wide mb-3 px-1">{title}</h2>
  ...
  const Row = ({ icon: Icon, label, desc, right, iconBg = 'bg-bg border-border' }: {
    icon: React.ElementType; label: string; desc?: string; right: React.ReactNode; iconBg?: string
  }) => (
    <div className="flex items-center gap-4 px-5 py-4 border-b last:border-0 border-border">
  ...
  return (
    <div className="min-h-screen bg-bg font-inter">
      <div className="bg-surface border-b border-border px-4 py-4 flex items-center gap-3 sticky top-0 z-10">
        ...
        <h1 className="text-base font-bold text-text-primary font-poppins">Settings</h1>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-6">
        ...
        <Section title="Privacy & Location">
          ...
          <Row icon={Shield} label="Profile Visibility" desc="Public to donors and NGOs" right={<ChevronRight className="w-4 h-4 text-text-secondary" />} />
          <Row icon={Lock} label="Change Password" right={<ChevronRight className="w-4 h-4 text-text-secondary" />} />
        </Section>
        ...
        <Section title="Appearance">
          ...
          <Row icon={Globe} label="Language" desc="English (India)" right={<ChevronRight className="w-4 h-4 text-text-secondary" />} />
        </Section>

        <Section title="Account">
          <Row icon={Smartphone} label="Connected Devices" desc="1 device signed in" right={<ChevronRight className="w-4 h-4 text-text-secondary" />} />
          <Row icon={Trash2} label="Delete Account" desc="Permanently remove your data" right={<ChevronRight className="w-4 h-4 text-error" />} iconBg="bg-[#FFEBEE] border-[#FFCDD2]" />
        </Section>

        <div className="text-center py-4">
          <p className="text-xs text-text-secondary">FoodConnect v2.4.1</p>
          <p className="text-xs text-text-secondary mt-0.5">© 2025 FoodConnect. Made with ❤️ in India</p>
        </div>
      </div>
    </div>
  )
