    ...
    <div className="min-h-screen bg-bg font-inter p-4 lg:p-8">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <button onClick={() => onNavigate('post-donation')} className="flex items-center justify-center gap-2 bg-primary text-white text-sm font-semibold px-4 py-2.5 rounded-xl shadow-md hover:bg-primary-dark transition-all w-full sm:w-auto">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-3">
            {quickActions.map((q) => (
              <button
                ...
                className="flex flex-col items-center gap-2 bg-surface rounded-2xl border border-border py-4 sm:py-5 px-2 sm:px-3 hover:shadow-md hover:-translate-y-0.5 transition-all cursor-pointer"
              >
                <div className={`w-10 h-10 sm:w-11 sm:h-11 rounded-xl ${q.color} flex items-center justify-center`}>
            ...
            {donations.length === 0 ? (
              ...
              donations.slice(0, 5).map((d, i) => {
                ...
                return (
                  <div
                    key={d.id}
                    className={`flex items-center gap-3 sm:gap-4 p-3.5 sm:p-4 hover:bg-bg cursor-pointer hover:border-primary transition-all ${i < Math.min(donations.length, 5) - 1 ? 'border-b border-border' : ''}`}
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-text-secondary mt-0.5 truncate">{d.pickupAddress} · {d.estimatedServings} servings</p>
                    </div>
        ...
        <div className="bg-surface rounded-2xl border border-border p-5 sm:p-6 shadow-sm">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
            {[
              ...
              <div key={item.label} className="text-center p-2 bg-bg/50 rounded-xl sm:bg-transparent">
