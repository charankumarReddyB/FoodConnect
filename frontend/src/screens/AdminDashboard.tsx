...
import { checkInApi, CheckInResponse } from '../services/api'
...
export default function AdminDashboard({ onNavigate }: AdminDashboardProps) {
  const [activeTab, setActiveTab] = useState<'analytics' | 'checkins'>('analytics')
  
  // TASK 3 — Admin Check-in state & filtering
  ...
  // Fetch admin check-in records (TASK 3 Endpoint: /api/v1/admin/checkins)
  ...
  // TASK 3 — Action: Admin manual mark user as checked-in
  ...
  // TASK 3 — Action: Admin undo check-in
  ...
  return (
    <div className="min-h-screen bg-bg font-inter">
      ...
      <div className="max-w-6xl mx-auto px-4 lg:px-8 py-6 space-y-8">
        ...
        {actionMessage && (
          <div className="bg-[#B71C1C] text-white text-sm px-4 py-3 rounded-2xl shadow-lg flex items-center justify-between animate-in fade-in">
        ...
        {/* TASK 3 — CHECK-IN MANAGEMENT TAB VIEW */}
        {activeTab === 'checkins' ? (
      ...
      {/* MANUAL MARK CHECK-IN MODAL */}
      {showManualModal && (
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in">
          <div className="bg-surface border border-border rounded-3xl p-6 w-full max-w-md shadow-2xl relative">
            <div className="flex items-center justify-between mb-4 border-b border-border pb-3">
              ...
              <button
                ...
                className="p-1 hover:bg-bg rounded-lg text-text-secondary"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleManualCheckIn} className="space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                <button
                  type="button"
                  onClick={() => setShowManualModal(false)}
                  className="flex-1 py-2.5 rounded-xl border border-border text-xs font-semibold text-text-secondary hover:bg-bg"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="flex-1 py-2.5 rounded-xl bg-[#B71C1C] text-white text-xs font-bold shadow hover:bg-[#880E4F] flex items-center justify-center gap-2 disabled:opacity-50"
                >
                  {isSubmittingManual ? (
                    <>
